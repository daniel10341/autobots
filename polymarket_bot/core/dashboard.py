...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .mode-stopped { background: #2a2a2a; color: #888; border: 1px solid #555; }
...
async function switchMode(mode) {
  try {
    ...
    const labels = {simulate: 'Restarting in Simulation...', live: 'Going Live...', stop: 'Stopping...'};
    showToast(labels[mode] || 'Switching...', false);
    ...
    if (result.ok) {
      setTimeout(() => showToast(mode === 'stop' ? 'Bot Stopped' : 'Bot Started — ' + mode.toUpperCase(), false), 2000);
    } else {
...
async function fetchData() {
  try {
    ...
    else if (d.mode === 'STOPPED') { badge.className = 'mode-badge mode-stopped'; badge.textContent = 'STOPPED'; }
    else { badge.className = 'mode-badge mode-dry'; badge.textContent = d.mode || 'DRY RUN'; }

    // Button states
    const isStopped = d.mode === 'STOPPED';
    ...
    document.getElementById('btn-stop').classList.toggle('active', isStopped);
    ...
    if (isStopped) {
      pulseEl.style.color = '#888';
      pulseEl.nextSibling.textContent = ' Stopped';
    } else if (d.mode === 'LIVE') {
      pulseEl.style.color = '#ff4444';
      pulseEl.nextSibling.textContent = ' Live';
    } else {
      pulseEl.style.color = '#00ff88';
      pulseEl.nextSibling.textContent = ' Running';
    }
  } catch(e) {
